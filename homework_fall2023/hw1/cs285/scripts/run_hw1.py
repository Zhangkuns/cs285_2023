...
def run_training_loop(params):
    ...
    # # ==== Quick expert eval (可选) ====
    # print('\n[Quick Check] Evaluating expert policy...')
    # expert_paths = utils.sample_n_trajectories(env, expert_policy, 5, params['ep_len'], render=False)
    # expert_returns = [np.sum(p["reward"]) for p in expert_paths]
    # print(f'Expert AverageReturn: {np.mean(expert_returns):.4f}')
    # print(f'Expert StdReturn    : {np.std(expert_returns):.4f}')
    # print(f'Expert Max/Min      : {np.max(expert_returns):.4f} / {np.min(expert_returns):.4f}\n')
    ...
    for itr in range(params['n_iter']):
        ...
        if itr == 0:
            ...
            # 直接加载专家的离线数据，执行标准 Behavior Cloning (BC)。
            ...
            # 智能体与环境交互收集新的轨迹；
            # 使用专家策略重新标注每个状态的动作；
            # 实现 DAgger 算法的核心思想：逐步修正智能体的错误分布。
        ...
        for _ in range(params['num_agent_train_steps_per_iter']):
          ...
          # 从经验缓冲区随机采样训练数据；
          # 执行策略网络的前向传播与梯度更新；
          # update() 计算监督学习损失（通常是MSE）。
          # ob_batch, ac_batch = TODO
          # 从经验缓冲区随机采样 train_batch_size 个数据点
          idxs = np.random.permutation(replay_buffer.obs.shape[0])[:params['train_batch_size']]
        ...
        # log/save
        # 视频日志：每隔 video_log_freq 次迭代，采样评估轨迹并保存视频；
        # 标量日志：记录训练与评估回报、步数、时间、损失等；
        # TensorBoard 输出：通过 logger.log_scalar()。

...
from hw3.cs285.infrastructure.replay_buffer import MemoryEfficientReplayBuffer, ReplayBuffer
...
def run_training_loop(config: dict, logger: Logger, args: argparse.Namespace):
    ...
    for step in tqdm.trange(config["total_steps"], dynamic_ncols=True):
        ...
        if isinstance(replay_buffer, MemoryEfficientReplayBuffer):
            ...
            replay_buffer.insert(action=action, reward=reward, next_observation=np.asarray(next_observation)[-1, ...], done=done and not truncated)
        else:

...
import hw4.cs285.infrastructure.pytorch_util as ptu
...
class SoftActorCritic(nn.Module):
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            target_values: torch.Tensor = reward + self.discount * next_qs * (1 - done.int())
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        num_samples = 10
        sampled_act = action_distribution.rsample((num_samples,))
        logp = action_distribution.log_prob(sampled_act)
        return torch.mean(-logp, dim=0)

    def actor_loss_reinforce(self, obs: torch.Tensor):
        batch_size = obs.shape[0]

        # TODO(student): Generate an action distribution
        action_distribution: torch.distributions.Distribution = self.actor(obs=obs)
        ...
        with torch.no_grad():
            ...
            # q_values = torch.stack([critic(obs.repeat(self.num_actor_samples, 1), action.reshape(-1, self.action_dim)).view(self.num_actor_samples, batch_size)for critic in self.critics],dim=0)
    ...
    def actor_loss_reparametrize(self, obs: torch.Tensor):
        ...
        #     [critic(obs, action) for critic in self.critics],
        #     dim=0
        # )
        # q_values = torch.mean(q_values, dim=0)
        q_values = self.critic(obs, action)
    ...
    def actor_loss_entropy_only(self, obs: torch.Tensor):
        """仅最大化 entropy，用于 sanity test"""
        action_distribution = self.actor(obs)
        entropy = torch.mean(self.entropy(action_distribution))
        loss = obs.new_zeros(1).sum()
        return loss, entropy
    ...
    def update_actor(self, obs: torch.Tensor):
        ...
        elif self.actor_gradient_type == "entropy_only":  # ✅ 添加这个
            loss, entropy = self.actor_loss_entropy_only(obs)

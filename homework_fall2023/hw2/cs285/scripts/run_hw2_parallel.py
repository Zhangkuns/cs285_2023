...
from hw2.cs285.agents.pg_agent import PGAgent
...
from hw2.cs285.infrastructure.action_noise_wrapper import ActionNoiseWrapper
import gym
...
def run_training_loop(args):
    ...
    # make the gym environment (create vectorized environments)
    def make_env(env_name, seed, rank):
        def _init():
            env_made = gym.make(env_name)
            env_made.seed(seed + rank)
            return env_made

        return _init

    num_envs = 5  # 设置并行环境的数量
    env_fns = [make_env(args.env_name, args.seed, i) for i in range(num_envs)]
    env = AsyncVectorEnv(env_fns)
    eval_env = gym.make(args.env_name)
    eval_env.reset(seed=args.seed + 999)  # 给个不同的 seed，保证评估稳定
    ...
    ac_dim = act_space.n if discrete else int(np.prod(act_space.shape))
    ...
    if hasattr(env, "model"):
        ...
        if isinstance(env, AsyncVectorEnv):
            try:
                # AsyncVectorEnv：到子进程取属性
                fps = env.get_attr("metadata")[0].get("render_fps", 30)
            except Exception:
                fps = 30
        else:
            # 普通单环境
            fps = getattr(getattr(env, "metadata", {}), "get", lambda *a, **k: 30)("render_fps", 30)
    ...
    for itr in range(args.n_iter):
        ...
        trajs, envsteps_this_batch = utils.sample_trajectories_parallel(env, agent.actor, args.batch_size, max_ep_len)
        ...
        if itr % args.scalar_log_freq == 0:
            ...
            eval_trajs, eval_envsteps_this_batch = utils.sample_trajectories(
                eval_env, agent.actor, args.eval_batch_size, max_ep_len
            )
...
def main():
    ...
    # Your other argument settings here...

...
class IQLAgent(AWACAgent):
    ...
    # -------- IQL: Update Q with target = r + γ V(s') --------
    def update_q(
        self,
        observations: torch.Tensor,
        actions: torch.Tensor,
        ...
        with torch.no_grad():
            target_values = rewards + self.discount * (1 - dones.int()) * v_next
    ...
    # -------- IQL: Expectile regression loss for V --------
    @staticmethod
    def iql_expectile_loss(
        ...
        u = target_qs - vs  # (B,1)
        weight = torch.where(u >= 0, expectile, 1.0 - expectile)

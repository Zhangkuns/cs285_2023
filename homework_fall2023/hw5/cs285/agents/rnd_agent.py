...
class RNDAgent(DQNAgent):
    ...
    def update(
        ...
        with torch.no_grad():
            ...
            tgt = self.rnd_target_net(next_observations)
            pred = self.rnd_net(next_observations)
            rnd_error = torch.norm(pred - tgt, dim=-1, keepdim=False)
            mean = rnd_error.mean()
            std = rnd_error.std(unbiased=False)
            rnd_error = (rnd_error - mean) / (std + 1e-8)
            # print("rnd_error", rnd_error.shape)

...
\begin{enumerate}
    ...
    \[
    b(s,a) = -\log\frac{\pi(a \mid s)}{\pi_\beta(a \mid s)}
    = \log\frac{\pi_\beta(a \mid s)}{\pi(a \mid s)}.
    \]
    ...
    给出一般的 $f$-散度约束下的**奖励加成** $b(s,a)$：
    
    从拉格朗日形式

    $$
    \max_{\pi}\; \mathbb{E}_{s,a\sim p^\pi}[Q(s,a)]-\lambda\,\mathbb{E}_{s\sim p^\pi}\mathbb{E}_{a\sim \pi_\beta(\cdot|s)}\!\left[f\!\left(\frac{\pi(a|s)}{\pi_\beta(a|s)}\right)\right]
    $$

    出发，用换测度把内层期望改到 $\pi$ 上（重要性采样）：

    $$
    \mathbb{E}_{a\sim \pi_\beta}[f(\tfrac{\pi}{\pi_\beta})]
    =\mathbb{E}_{a\sim \pi}\!\left[\frac{\pi_\beta(a|s)}{\pi(a|s)}\, f\!\left(\frac{\pi(a|s)}{\pi_\beta(a|s)}\right)\right].
    $$

    于是目标可写为

    $$
    \mathbb{E}_{s,a\sim p^\pi}\!\Big[\,Q(s,a)\;-\;\lambda\frac{\pi_\beta(a|s)}{\pi(a|s)}\, f\!\Big(\frac{\pi(a|s)}{\pi_\beta(a|s)}\Big)\Big].
    $$
    ...
    $$
    \boxed{\;
    b(s,a)\;=\;-\;\frac{\pi_\beta(a|s)}{\pi(a|s)}\; f\!\left(\frac{\pi(a|s)}{\pi_\beta(a|s)}\right)\; }.
    $$
    ...
    你已经把**轨迹级 KL**化到了“只看状态序列”的形式：

$$
D_{\text{KL}}\!\big(p^\pi(\tau)\,\|\,p^{\pi_\beta}(\tau)\big)
=\mathbb{E}_{\tau\sim p^\pi}\!\Big[\sum_{t=1}^{H-1}\log\frac{p^\pi(s_{t+1}\mid s_t)}{p^{\pi_\beta}(s_{t+1}\mid s_t)}\Big].
$$

接下来要做的，就是把这个“**对轨迹求和的状态转移 KL**”改写成**一步一步的（s,a,s′）期望**，从而得到能加到奖励里的 $b(s,a)$。下面按步骤推：

---

1) 从动作边缘化得到状态转移分布

由提示先对动作做边缘化，定义“在策略 $\pi$ 下的**状态转移分布**”：

$$
p^\pi(s'\mid s)=\int \pi(a\mid s)\,p(s'\mid s,a)\,da,\qquad
p^{\pi_\beta}(s'\mid s)=\int \pi_\beta(a\mid s)\,p(s'\mid s,a)\,da.
$$

把轨迹级 KL 写成这些 one-step 转移分布的对数比之和（这一步你已完成）：

$$
D_{\text{KL}}(p^\pi\|\;p^{\pi_\beta})
=\mathbb{E}_{\tau\sim p^\pi}\!\Big[\sum_{t}\log\frac{p^\pi(s_{t+1}\mid s_t)}{p^{\pi_\beta}(s_{t+1}\mid s_t)}\Big].
$$

---

2) 从“对轨迹的期望”变成“一步联合分布的期望”

在策略 $\pi$ 下，一步联合分布为

$$
(s_t,a_t,s_{t+1})\sim p^\pi(s_t)\,\pi(a_t\mid s_t)\,p(s_{t+1}\mid s_t,a_t).
$$

注意上式中的**被加惩罚的项仅依赖 $(s_t,s_{t+1})$**，与 $a_t$ 无关（因为我们已经把动作边缘化进 $p^\pi(s'|s)$ 里了）。
用全期望公式把“对轨迹求和的期望”换成“一步一步对 $(s,a,s')$ 的期望”：

$$
D_{\text{KL}}(p^\pi\|\;p^{\pi_\beta})
=\mathbb{E}_{s\sim p^\pi,\;a\sim\pi(\cdot|s),\;s'\sim p(\cdot|s,a)}
\!\Big[\log\tfrac{p^\pi(s'\mid s)}{p^{\pi_\beta}(s'\mid s)}\Big].
$$
...
我们要在目标里**减去** $\lambda D_{\text{KL}}$（拉格朗日惩罚）：

$$
-\lambda\,D_{\text{KL}}(p^\pi\|\;p^{\pi_\beta})
=\mathbb{E}_{s,a,s'}\!\Big[\lambda\;\underbrace{\log\tfrac{p^{\pi_\beta}(s'\mid s)}{p^\pi(s'\mid s)}}_{\text{这就是一步的“奖励加成”}}\Big].
$$

于是，一步层面的“**过渡级加成**”可以定义为

$$
\boxed{\,b(s,a,s')=\log\frac{p^{\pi_\beta}(s'\mid s)}{p^\pi(s'\mid s)}\, }.
$$

这里取的是 $\log\frac{p^{\pi_\beta}}{p^\pi}$，是因为我们在目标里减 $\lambda D_{\text{KL}}$，
而 $D_{\text{KL}}$ 的被积项是 $\log\frac{p^\pi}{p^{\pi_\beta}}$；加到奖励里要取负号，故为上式。

---

4) 从 $b(s,a,s')$ 变成能与 $r(s,a)$ 同用的 $b(s,a)$

奖励是定义在 $(s,a)$ 上的，所以把上面的过渡级加成对 $s'$ 取期望（在真实动力学下）：

$$
\boxed{\,b(s,a)=\mathbb{E}_{s'\sim p(\cdot\mid s,a)}
\!\left[\log\frac{p^{\pi_\beta}(s'\mid s)}{p^\pi(s'\mid s)}\right].\,}
$$
...
在 Bellman 目标里用 $\bar r$ 就等价于在优化里施加了“**限制状态轨迹分布偏移**”的轨迹级 KL 惩罚。
\end{enumerate}
...
python hw5/cs285/scripts/run_hw5_explore.py \
    -cfg hw5/experiments/exploration/pointmass_easy_random.yaml \
    --dataset_dir hw5/datasets/
python hw5/cs285/scripts/run_hw5_explore.py \
    -cfg hw5/experiments/exploration/pointmass_medium_random.yaml \
    --dataset_dir hw5/datasets/
python hw5/cs285/scripts/run_hw5_explore.py \
    -cfg hw5/experiments/exploration/pointmass_hard_random.yaml \
    --dataset_dir hw5/datasets/
\end{lstlisting}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.32\linewidth}
        \centering
        \includegraphics[width=\linewidth]{PointmassEasy-v0_random.png}
        \caption{Random Easy}
        \label{fig:PointmassEasy-v0_random}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\linewidth}
        \centering
        \includegraphics[width=\linewidth]{PointmassMedium-v0_random.png}
        \caption{Random Medium}
        \label{fig:PointmassMedium-v0_random}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\linewidth}
        \centering
        \includegraphics[width=\linewidth]{PointmassHard-v0_random.png} % 第三张图的文件名
        \caption{Random Hard}
        \label{fig:PointmassHard-v0_random}
    \end{subfigure}
    \caption{Visualizations of datasets collected by random policy in the three environments.}
    \label{fig:pointmass_random}
\end{figure}
...
\begin{lstlisting}[language=bash]
python hw5/cs285/scripts/run_hw5_explore.py \
    -cfg hw5/experiments/exploration/pointmass_easy_rnd.yaml \
    --dataset_dir hw5/datasets/
python hw5/cs285/scripts/run_hw5_explore.py \
    -cfg hw5/experiments/exploration/pointmass_medium_rnd.yaml \
    --dataset_dir hw5/datasets/
python hw5/cs285/scripts/run_hw5_explore.py \
    -cfg hw5/experiments/exploration/pointmass_hard_rnd.yaml \
    --dataset_dir hw5/datasets/
\end{lstlisting}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.32\linewidth}
        \centering
        \includegraphics[width=\linewidth]{PointmassEasy-v0_rnd1.0.png}
        \caption{RND Easy}
        \label{fig:PointmassEasy-v0_rnd1.0}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\linewidth}
        \centering
        \includegraphics[width=\linewidth]{PointmassMedium-v0_rnd1.0.png}
        \caption{RND Medium}
        \label{fig:PointmassMedium-v0_rnd1.0}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\linewidth}
        \centering
        \includegraphics[width=\linewidth]{PointmassHard-v0_rnd1.0.png} % 第三张图的文件名
        \caption{RND Hard}
        \label{fig:PointmassHard-v0_rnd1.0}
    \end{subfigure}
    \caption{Visualizations of datasets collected by rnd policy in the three environments.}
    \label{fig:pointmass_rnd1.0}
\end{figure}
...
\begin{lstlisting}[language=bash]
python hw5/cs285/scripts/run_hw5_offline.py \
    -cfg hw5/experiments/offline/pointmass_easy_cql.yaml \
    --dataset_dir hw5/datasets/
python hw5/cs285/scripts/run_hw5_offline.py \
    -cfg hw5/experiments/offline/pointmass_easy_dqn.yaml \
    --dataset_dir hw5/datasets/

python hw5/cs285/scripts/run_hw5_offline.py \
    -cfg hw5/experiments/offline/pointmass_medium_cql.yaml \
    --dataset_dir hw5/datasets/
python hw5/cs285/scripts/run_hw5_offline.py \
    -cfg hw5/experiments/offline/pointmass_medium_dqn.yaml \
    --dataset_dir hw5/datasets
python hw5/cs285/scripts/run_hw5_offline.py \
    -cfg hw5/experiments/offline/pointmass_medium_iql.yaml \
    --dataset_dir hw5/datasets
\end{lstlisting}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{cql_0.1_easy_eval_return.png}
        \caption{Eval return Curve}
        \label{fig:cql_0.1_eval_return}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{cql_0.1_easy_q_values.png}
        \caption{Q Values Curve}
        \label{fig:cql_0.1_q_values}
    \end{subfigure}
    \caption{Learning Curves for CQL 0.1.}
    \label{fig:cql_0.1_curves}
\end{figure}
明显可以看到，CQL 的正则（α > 0）使得 Q 值分布收敛到一个更“保守”的范围内，减少了虚高估计。

这正是 CQL 的目标——牺牲部分乐观性换取更稳健的策略评估

\begin{verbatim}
    python hw5/cs285/scripts/run_hw5_offline.py \
    -cfg hw5/experiments/offline/pointmass_medium_cql_1.yaml \
    --dataset_dir hw5/datasets/

    python hw5/cs285/scripts/run_hw5_offline.py \
    -cfg hw5/experiments/offline/pointmass_medium_cql_5.yaml \
    --dataset_dir hw5/datasets/

    python hw5/cs285/scripts/run_hw5_offline.py \
    -cfg hw5/experiments/offline/pointmass_medium_cql_10.yaml \
    --dataset_dir hw5/datasets/
\end{verbatim}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{cql_10_medium_eval_return.png}
        \caption{Eval return Curve}
        \label{fig:cql_10_medium_eval_return}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{cql_10_medium_q_values.png}
        \caption{Q Values Curve}
        \label{fig:cql_10_medium_q_values}
    \end{subfigure}
    \caption{Learning Curves for CQL 10.}
    \label{fig:cql_10_medium_curves}
\end{figure}
在 PointmassMedium-v0 上比较 CQL 不同 α 与 DQN：α=0.1 收敛更快且最终性能最佳；α=1,5,10 过度保守导致性能显著下降。说明 CQL 的 α 控制了对 OOD 动作的惩罚强度，中等 α 能有效抑制过估计并提升离线性能。
...
\begin{lstlisting}[language=bash]
    python hw5/cs285/scripts/run_hw5_offline.py \
        -cfg hw5/experiments/offline/pointmass_hard_awac.yaml \
        --dataset_dir hw5/datasets/
    python hw5/cs285/scripts/run_hw5_offline.py \
        -cfg hw5/experiments/offline/pointmass_hard_iql.yaml \
        --dataset_dir hw5/datasets/
    python hw5/cs285/scripts/run_hw5_offline.py \
        -cfg hw5/experiments/offline/pointmass_hard_cql.yaml \
        --dataset_dir hw5/datasets/
    python hw5/cs285/scripts/run_hw5_offline.py \
        -cfg hw5/experiments/offline/pointmass_hard_cql_1.yaml \
        --dataset_dir hw5/datasets/
    python hw5/cs285/scripts/run_hw5_offline.py \
        -cfg hw5/experiments/offline/pointmass_hard_dqn.yaml \
        --dataset_dir hw5/datasets/
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{awac_iql_eval_return.png}
        \caption{Eval return Curve}
        \label{fig:awac_iql_eval_return}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{awac_iql_q_values.png}
        \caption{Q Values Curve}
        \label{fig:awac_iql_q_values}
    \end{subfigure}
    \caption{Learning Curves for AWAC and IQL.}
    \label{fig:awac_iql_curves}
\end{figure}
...
\begin{verbatim}
    # 采集数据
    python hw5/cs285/scripts/run_hw5_explore.py \
    -cfg hw5/experiments/exploration/pointmass_hard_rnd1k.yaml \
    --dataset_dir hw5/datasets/
    python hw5/cs285/scripts/run_hw5_explore.py \
    -cfg hw5/experiments/exploration/pointmass_hard_rnd5k.yaml \
    --dataset_dir hw5/datasets/
    python hw5/cs285/scripts/run_hw5_explore.py \
    -cfg hw5/experiments/exploration/pointmass_hard_rnd.yaml \
    --dataset_dir hw5/datasets/
    python hw5/cs285/scripts/run_hw5_explore.py \
    -cfg hw5/experiments/exploration/pointmass_hard_rnd20k.yaml \
    --dataset_dir hw5/datasets/
    # 训练CQL
    python hw5/cs285/scripts/run_hw5_offline.py \
    -cfg hw5/experiments/offline/pointmass_hard_cql1k.yaml \
    --dataset_dir hw5/datasets/ &&\
    python hw5/cs285/scripts/run_hw5_offline.py \
    -cfg hw5/experiments/offline/pointmass_hard_cql5k.yaml \
    --dataset_dir hw5/datasets/
    ...
    --dataset_dir hw5/datasets/ &&\
    python hw5/cs285/scripts/run_hw5_offline.py \
    -cfg hw5/experiments/offline/pointmass_hard_cql20k.yaml \
    --dataset_dir hw5/datasets/
\end{verbatim}
\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{cql_data_number_eval_return.png}
        \caption{Eval return Curve}
        \label{fig:cql_data_number_eval_return}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{cql_data_number_q_values.png}
        \caption{Q Values Curve}
        \label{fig:cql_data_number_q_values}
    \end{subfigure}
    \caption{Learning Curves for CQL with Different Data Sizes.}
    \label{fig:cql_data_number_curves}
\end{figure}
明显看到，当数据量不足的时候完全无法训练起来，随着数据量增加，性能逐渐提升，20k 数据时接近最优。
...
\textbf{Solution}:
\begin{verbatim}
    python hw5/cs285/scripts/run_hw5_finetune.py \
    -cfg hw5/experiments/finetuning/pointmass_hard_cql_finetune.yaml \
    --dataset_dir hw5/datasets/
    python hw5/cs285/scripts/run_hw5_finetune.py \
    -cfg hw5/experiments/finetuning/pointmass_hard_cql_0.1_finetune.yaml \
    --dataset_dir hw5/datasets/
    python hw5/cs285/scripts/run_hw5_finetune.py \
    -cfg hw5/experiments/finetuning/pointmass_hard_cql_finetune_ai.yaml \
    --dataset_dir hw5/datasets/
\end{verbatim}
\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{finetune_eval_return.png}
        \caption{Eval return Curve}
        \label{fig:finetune_eval_return}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{finetune_q_values.png}
        \caption{Q Values Curve}
        \label{fig:finetune_q_values}
    \end{subfigure}
    \caption{Learning Curves for CQL with finetuning.}
